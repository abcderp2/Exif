...
JPEG、PNG、WebPを端末のブラウザ内だけで一括処理し、入力画像に含まれていたExif、GPS、撮影日時、機種名、作者情報、コメントなどを出力へ引き継がないための静的Webアプリです。
...
保存ファイル名は、従来のclean付き、元の名前を維持、元の名前を使わない匿名連番から選べます。元の名前を維持する場合も、パス区切りや制御文字などは安全な文字へ置き換えます。匿名化を選ぶと、画像と分析レポートの名前に元のファイル名を記録しません。
...
画像をブラウザ画面へドラッグしている間は追加先を全画面で表示します。画面の別の場所へドロップしても、ブラウザが画像ファイルへ移動しないように抑止し、そのまま一覧へ追加します。フォルダーと画像以外の項目は受け付けません。
...
reports.jsは一括JSONとCSVの生成、CSV数式注入対策を担当します。画像処理とは独立した純粋な処理にして、無料プランの一般的なAIでも変更箇所を追いやすくしています。
